...
from tool.VRE_Tool import dorotheaTool
...
class Wrapper:
    ...
    def run(self, input_files, input_metadata, output_files, output_metadata):
        """
        Main run function for running DoRothEA tool.
...
def main_wrapper(config_path, in_metadata_path, out_metadata_path):
    ...
    try:
        ...
        logger.progress("DoRothEA tool successfully executed; see {}".format(out_metadata_path))
        return result

    except Exception as error:
        errstr = "DoRothEA tool wasn't successfully executed. ERROR: {}.".format(error)
        logger.error(errstr)
        raise Exception(errstr)


if __name__ == "__main__":
    PARSER = argparse.ArgumentParser(description="VRE DoRothEA Tool")
